::User.all.each do |user|
  if user.plugins.where(:name => 'refinery_submissions').blank?
    user.plugins.create(:name => "refinery_submissions",
...
if defined?(::Page)
  ...
  contact_us_page = ::Page.create({
    :title => "Contact",
    :link_url => "/contact",
    :menu_match => "^/(submissions|contact).*$",
    :deletable => false,
  ...
  thank_you_page = contact_us_page.children.create({
    ...
    :menu_match => "^/(submissions|contact)/thank_you$",
    :show_in_menu => false,
    :deletable => false,
  ...
  thank_you_page.parts.create({
    :title => "Body",
    :body => "<p>We've received your contact form submission and will get back to you with a response shortly.</p><p><a href='/'>Return to the home page</a></p>",
    :position => 0
  })
